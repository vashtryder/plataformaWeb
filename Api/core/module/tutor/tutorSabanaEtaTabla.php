<?php
    ...
    $return_arr    = array();
    $data = array($_SESSION['tutor']['grado'],$_SESSION['tutor']['seccion'],$_SESSION['tutor']['nivel'],$_SESSION['colegio'][0],$_SESSION['anio'][0]);
    $rs = gestorEstudiante::get_estudiante_reporte($data);
    foreach ($rs as $rows){
        $rows1 = gestorEstudiante::set_estudiante($rows[3]);
        $row_array['id']         = $rows1[0];
        $row_array['estudiante'] = $rows1[3]." ".$rows1[4]." ".$rows1[5];
        $row_array['foto']       = $rows1[14];
